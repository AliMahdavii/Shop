document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!loggedInUser) {
    cartContainer.innerHTML = `
      ...
      <a href="login.html" class="back-btn">ورود</a>
    `;
    return;
  }
  ...
  if (cart.length === 0) {
    cartItemContainer.innerHTML = `<p class="empty-p">سبد خرید شما خالی است 🛒</p>`;
    return;
  }
    ...
    cartItemContainer.insertAdjacentHTML(
      ...
      <div class="shopping-card" onclick="openProduct('${id}')">
        ...
        <div class="buttons">
          ...
          <button onclick="event.stopPropagation(), deleteItem(${id})" class="deletItem">حذف</button>
        </div>
      </div>
      `
    );
...
function deleteItem(id) {
  const loggedInUser = JSON.parse(localStorage.getItem("logged-in"))?.username;
  let cartData = JSON.parse(localStorage.getItem(loggedInUser)) || { cart: [] };

  cartData.cart = cartData.cart.filter((item) => item.id !== id);
  localStorage.setItem(loggedInUser, JSON.stringify(cartData));
